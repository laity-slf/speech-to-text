...
class ASRExecutor(BaseExecutor):
    ...
    @staticmethod
    def video_2_audio(input_file, output_file, start_time=None, end_time=None,transform_type =None):
        """
        视频转音频
        """
        order = '-vn -ar 16000 -ac 1 -ab 256 -f wav' if transform_type=='mp4' else '-ar 16000 -ac 1 -ab 256'
        if start_time and end_time:
            order += ' -ss ' + start_time + ' -t ' + end_time
        ff = ffmpy.FFmpeg(
            inputs={input_file: None},
            outputs={output_file: order}
        )
        # ff.cmd
        ff.run()

    def read_to_infer(self,
        ...
        file_name = os.path.dirname(audio_file)
        base_name = os.path.basename(audio_file)
        temp_wav = os.path.join(file_name,'temp_wav')
        if not os.path.exists(temp_wav):os.mkdir(temp_wav)
        a_e =os.listdir(temp_wav)
        if base_name.endswith('.mp4'):
            output_file = os.path.join(temp_wav,base_name.replace('.mp4', '.wav'))
            if os.path.basename(output_file) not in a_e:
                self.video_2_audio(audio_file, output_file, start_time='00:00:00', end_time='00:00:40',transform_type='mp4')
        elif base_name.endswith('.wav'):
            output_file=os.path.join(temp_wav,base_name)
            if os.path.basename(output_file) not in a_e:
                self.video_2_audio(audio_file, output_file , start_time='00:00:00', end_time='00:00:40', transform_type='wav')
        else:
            raise Exception('Invalid audio_file or audio type')
        ...
        self.preprocess(model, output_file)
        self.infer(model)
